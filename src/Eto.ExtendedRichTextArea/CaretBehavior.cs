...
class CaretBehavior
{
	...
	public void SetIndex(int value, bool updateSelectionAttributes)
	{
	...
	public void Navigate(DocumentNavigationMode mode)
	{
		...
		SetIndex(Document.Navigate(_caretIndex, mode, location), true);
	...
	{
		if (_caretIndex > _textArea.Document.Length)
		{
			SetIndex(_textArea.Document.Length, false);
